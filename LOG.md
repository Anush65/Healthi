...
* **Phase 9/10/11: Firebase Migration & Doctor Role**: Completely migrated the storage layer from local IndexedDB to Firebase Firestore. Introduced Firebase Authentication allowing users to register as either a "Patient" or "Doctor." Built a new "Doctor Dashboard" allowing physicians to search patients by UID and view their chronological wellness ledgers and dynamic condition charts in real-time. Removed local print button from patient export to favor live digital tracking.
* **Phase 12/13: Google Auth Migration**: Replaced Email/Password authentication entirely with Google Sign-In (`GoogleAuthProvider`). Implemented an interception flow that prompts new users to select their role ("Patient" or "Doctor") immediately following their first OAuth login.
